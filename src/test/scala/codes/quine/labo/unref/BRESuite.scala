...
class BRESuite extends munit.FunSuite:
  test("BRE.parse") {
    ...
    assertEquals(BRE.parse("x*"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Star)))
    assertEquals(BRE.parse("x+"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Plus)))
    assertEquals(BRE.parse("x?"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Question)))
    assertEquals(BRE.parse("x{1}"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Exact(1))))
    assertEquals(BRE.parse("x{1,}"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Unbounded(1))))
    assertEquals(BRE.parse("x{1,2}"), Some(BRE.Rep(BRE.Lit('x'), Quantifier.Bounded(1, 2))))
